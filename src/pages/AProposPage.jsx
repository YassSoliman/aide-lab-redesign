    ...
    <section ref={ref} className="section-padding bg-white">
      <div className="container-custom">
        ...
        <div className="relative">
          {/* Vertical line */}
          <div className="absolute left-4 md:left-1/2 top-0 bottom-0 w-px bg-ink/10 md:-translate-x-px" />
              ...
              >
                <div className={`md:w-1/2 ${i % 2 === 0 ? 'md:text-right md:pr-12' : 'md:text-left md:pl-12'}`}>
                  <div className="pl-12 md:pl-0">
                    <span className="text-2xl font-display text-coral">{item.year}</span>
                    <p className="text-ink/70 mt-1">{item.event}</p>
                  </div>
                </div>
                {/* Dot */}
                <div className="absolute left-4 md:left-1/2 top-2 w-3 h-3 rounded-full bg-coral -translate-x-1/2 ring-4 ring-white" />
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {founders.map((founder, i) => (
            <motion.div
              ...
              className="bg-white border border-ink/5 p-8"
            >
              <div className="w-20 h-20 rounded-full bg-coral/10 flex items-center justify-center mb-6 mx-auto">
                <User className="w-10 h-10 text-coral" />
              </div>
              <div className="text-center mb-4">
                <h3 className="font-display text-xl text-ink">{founder.name}</h3>
                <p className="text-sm font-mono text-sage-dark">{founder.title}</p>
              </div>
              <p className="text-ink/70 text-sm leading-relaxed text-center">{founder.bio}</p>
            </motion.div>
          ))}
        </div>
      </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 md:gap-8">
          {scanners.map((scanner, i) => (
            <motion.div
              ...
              className="bg-stone border border-ink/5 p-8 text-center"
            >
              <div className="inline-flex items-center justify-center w-14 h-14 mb-4 bg-ink/5 text-coral-dark">
                <Monitor className="w-6 h-6" />
              </div>
              <h3 className="font-display text-lg text-ink mb-2">{scanner.name}</h3>
              <p className="text-xs font-mono text-coral-dark mb-3">{scanner.brand}</p>
              <p className="text-ink/60 text-sm leading-relaxed">{scanner.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-4xl mx-auto">
          {values.map((value, i) => {
            ...
            return (
              ...
              >
                <div className="inline-flex items-center justify-center w-14 h-14 mb-4 bg-coral/10 text-coral">
